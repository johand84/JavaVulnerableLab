		...
		throws ServletException, IOException
	{
		...
		try {
			...
			if (con != null && !con.isClosed() && request.getParameter("send") != null) {
				...
				response.sendRedirect(
					request.getContextPath() +
					"/vulnerability/SendMessage.jsp?status=<b style='color:red'>* Something Went Wrong</b>"
				);
			}
		} catch (Exception ex) {
			response.sendRedirect(
				request.getContextPath() +
				"/vulnerability/SendMessage.jsp?status=<b style='color:red'>* Something Went Wrong</b><br/>" +
				ex
			);
		}
	}
